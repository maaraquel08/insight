...
import { AlertCircle, Check, Plus, X, CalendarIcon } from "lucide-react";
...
const filterAddButtonVariants = cva(
    [
        "inline-flex items-center shrink-0 justify-center transition shrink-0 text-foreground",
...
const filterFieldValueVariants = cva(
    "text-foreground transition shrink-0 flex items-center  relative focus-visible:z-1",
    {
        variants: {
            variant: {
                solid: "bg-secondary",
            ...
            size: {
                lg: "h-10 text-sm px-4 [&_svg:not([class*=size-])]:size-4",
                md: "h-9 text-sm px-3 [&_svg:not([class*=size-])]:size-4",
                sm: "h-8 text-xs px-2.5 [&_svg:not([class*=size-])]:size-3.5",
            },
            cursorPointer: {
...
export interface FilterFieldConfig<T = unknown> {
    ...
    // Date type options for nested date selection
    dateTypes?: Array<{
        key: string;
        label: string;
        type: string;
...
function FilterValueSelector<T = unknown>({
            ...
            return (
                <div className="flex items-center" data-slot="filters-item">
                    <FilterInput
                        ...
                        value={startTime}
                        onChange={(e) =>
                            onChange([e.target.value, endTime] as T[])
                        }
                    ...
                    <FilterInput
                        ...
                        value={endTime}
                        onChange={(e) =>
                            onChange([startTime, e.target.value] as T[])
                        }
                        onInputChange={field.onInputChange}
                        className={field.className}
                        field={field}
            ...
            return (
                <div className="flex items-center" data-slot="filters-item">
                    <FilterInput
                        ...
                        value={startDateTime}
                        onChange={(e) =>
                            onChange([e.target.value, endDateTime] as T[])
                        }
                        onInputChange={field.onInputChange}
                    ...
                    <FilterInput
                        ...
                        value={endDateTime}
                        onChange={(e) =>
                            onChange([startDateTime, e.target.value] as T[])
                        }
                        onInputChange={field.onInputChange}
                        className={cn("w-36", field.className)}
                        field={field}
                        ...
                        <Calendar
                            ...
                            onSelect={(date) => {
                                if (date) {
                                    ...
                                    onChange([formattedDate, endDate] as T[]);
                                    if (field.onInputChange) {
                                        ...
                                        field.onInputChange(syntheticEvent);
                                    }
                        ...
                        <Calendar
                            ...
                            onSelect={(date) => {
                                if (date) {
                                    const formattedDate =
                                        formatDateForInput(date);
                                    onChange([startDate, formattedDate] as T[]);
                                    if (field.onInputChange) {
                                        const syntheticEvent = {
                                            target: {
                                                value: formattedDate,
            ...
            return (
                <div className="flex items-center" data-slot="filters-item">
                    <FilterInput
                        ...
                        value={minVal}
                        onChange={(e) =>
                            onChange([e.target.value, maxVal] as T[])
                        }
                        onInputChange={field.onInputChange}
                    ...
                    <FilterInput
                        ...
                        value={maxVal}
                        onChange={(e) =>
                            onChange([minVal, e.target.value] as T[])
                        }
                        onInputChange={field.onInputChange}
    ...
    if (field.type === "date") {
        const dateValue = (values[0] as string) || "";
    ...
    return (
        ...
        >
            ...
            <PopoverContent
                className={cn("w-36 p-0", field.popoverContentClassName)}
            >
                <Command>
                    {field.searchable !== false && (
                        <CommandInput
...
export function Filters<T = unknown>({
    ...
    const [showDateTypeSelection, setShowDateTypeSelection] = useState(false);
    ...
    const addFilter = useCallback(
        (fieldKey: string) => {
            ...
            if (field && field.key) {
                // Check if this is a date field with nested date types
                if (field.dateTypes && field.dateTypes.length > 0) {
                    setSelectedDateField(field);
                    setShowDateTypeSelection(true);
                    return;
                }
            ...
            >
                {showAddButton && selectableFields.length > 0 && (
                    <Popover
                        ...
                        onOpenChange={(open) => {
                            ...
                            if (!open) {
                                setSelectedFieldForOptions(null);
                                setTempSelectedValues([]);
                                setShowDateTypeSelection(false);
                    ...
                    >
                        ...
                        >
                            <Command>
                                {showDateTypeSelection && selectedDateField ? (
                                    // Show date type selection
                                    <>
                                        {showSearchInput && (
                                            <CommandInput
                                        ...
                                        <CommandList>
                                            <CommandEmpty>
                                                {mergedI18n.noResultsFound}
                                            </CommandEmpty>
                                            <CommandGroup>
                                                {selectedDateField.dateTypes?.map(
                                                    (dateType) => {
                                                        // Check if this date type already has a filter
                                                        const hasFilter =
                                                            !allowMultiple &&
                                                            filters.some(
                                                                (filter) =>
                                                        ...
                                                        return (
                                                            <CommandItem
                                                                ...
                                                                onSelect={() => {
                                                                    // Create a filter for the selected date type
                                                                    const defaultOperator =
                                                                        "between";
                                                                    const defaultValues =
                                                                        [
                                    ...
                                    <>
                                        ...
                                        <CommandList>
                                            ...
                                            {fields.map((item, index) => {
                                                // Filter out date type fields from main selection (they're only shown in nested popover)
                                                // Check if this field is a date type (exists in any field's dateTypes array)
                                                const itemKey =
                                                    !isFieldGroup(item) &&
                                                    !isGroupLevelField(item)
                                                        ? (
                                                              item as FilterFieldConfig<T>
                                                          ).key
                                                        : undefined;
                                                const isDateType = itemKey
                                                    ? fields.some((field) => {
                                                          if (
                                                              isFieldGroup(
                                                                  field
                                                          ...
                                                          const fieldConfig =
                                                              field as FilterFieldConfig<T>;
                                                          return fieldConfig.dateTypes?.some(
                                                              (dt: {
                                                                  key: string;
                                                              }) =>
                                                                  dt.key ===
