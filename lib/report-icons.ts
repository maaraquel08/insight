 ...
 * This provides a sustainable, maintainable way to assign icons to reports.
 * 
 * To add new reports:
 * 1. Add the report title (case-insensitive) as a key
 * 2. Assign an appropriate icon from lucide-react
 * 
 * The mapping is case-insensitive for flexibility.
 */
const reportIconMap = new Map<string, LucideIcon>([
    // HR Reports
    ["overtime", Clock],
    ["leaves", Calendar],
...
export function getReportIcon(title: string): LucideIcon {
    ...
    return reportIconMap.get(normalizedTitle) || FALLBACK_ICON;
}
